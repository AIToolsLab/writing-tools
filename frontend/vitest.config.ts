...
export default defineConfig({
	...
	test: {
		// Our unit tests under src/, plus manifest/ (build-time config that isn't
		// app code and so doesn't belong under src/). Playwright owns
		// tests/*.spec.ts and has its own runner, so keep Vitest out of that
		// directory.
		include: ['src/**/*.{test,spec}.{ts,tsx}', 'manifest/**/*.test.ts'],
