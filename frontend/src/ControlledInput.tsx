...
export const stringIsNontrivial = (x: string | null) =>
	(x || '').trim().length > 0;
...
export const withValidation = (
	...
	return {
		...
		complete: (state: Record<string, string | null>) =>
			requiredInputs.every((name) => stringIsNontrivial(state[name])),
	};
};
...
export const NextBtn = (props: NextBtnProps) => {
	...
	return (
		<button
			...
			className={[
				...
				!enabled
					...
					: 'hover:bg-violet-50 focus:ring-2 focus:ring-violet-300',
			].join(' ')}
			onClick={() => {
		...
		>
			{props.children ||
				(enabled ? 'Next' : 'Please answer all prompts above')}
		</button>
	);
};
