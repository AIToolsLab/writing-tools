	...
	if (page === 'editor') {
		...
		if (!isValidOrder) {
			return (
				<div>
					{' '}
					Invalid condition order. Please use a unique combination of
					'e', 'q', and 'r'.{' '}
				</div>
			);
		}
		...
		if (page === 'study-consentForm') {
			...
			const redirectURL = encodeURIComponent(
				window.location.origin +
					`/editor.html?${nextUrlParams.toString()}`,
			);
			...
			return (
				<div className={classes.studyIntroContainer}>
					<h1>Welcome!</h1>
					<p>
						Thank you for agreeing to participate in our writing
						study. You will be working on three different writing
						tasks. Each task will have a different type of writing
						support. As you write, please pay attention to the
						suggestions the writing tool offers and use them when
						they seem helpful. There are no right or wrong ways to
						interact with the tool. Your responses will be kept
						confidential.
					</p>
					<button
						type="button"
			...
			return (
				<div className={classes.studyIntroContainer}>
					<p>
						{' '}
						Now you will start task {taskNumber} of 3. <br /> In
						this task, you will be using Suggestion {taskLabel}.{' '}
					</p>
					<button
						type="button"
						onClick={() => {
			...
			return (
				<div className={classes.studyIntroContainer}>
					<p>
						{' '}
						Thank you for completing Task {taskNumber} using
						Suggestion {taskLabel}. <br /> Please take a moment to
