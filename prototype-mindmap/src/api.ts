...
import type { GroundedClaim, GroundedRecap, MirrorClaim, MirrorReflection, SourceSpan } from "./types";
...
function systemPrompt(context: LLMContext, _config: MindmapConfig, repair?: StructuredRejection, transport: ProviderTransport = "chat_json"): string {
  ...
  const transportInstruction = transport === "responses_tools"
    ? "Use propose_reflection_v1 for reflections and propose_map_action_v1 for structural proposals. These tools only request review and never write to the map. For questions, asides, grounded recaps, grounded options, or suggestions, return the required conversational JSON output."
